...
export function parseDictationSegment(segment: string): VisionExtract | null {
  ...
  text = text.replace(/^(and|ja|plus|sekä)\s+/i, '').trim();
  if (!text) return null;
 ...
 * Optional placeLabel is stamped on each line for shelf-by-shelf walks.
 */
export function transcriptToFridgeDocument(
  transcript: string,
  options?: { title?: string; placeLabel?: string },
  ...
  for (const seg of segments) {
    ...
    if (line) {
      if (options?.placeLabel) {
        line.rawNotes = [line.rawNotes, `Place: ${options.placeLabel}`]
          .filter(Boolean)
          .join(' · ');
      }
      lines.push(line);
  ...
  if (lines.length === 0) {
    ...
    if (one) {
      if (options?.placeLabel) {
        one.rawNotes = [one.rawNotes, `Place: ${options.placeLabel}`]
          .filter(Boolean)
          .join(' · ');
      }
      lines.push(one);
  ...
  return {
    ...
    rawNotes: [
      `Dictated: ${cleaned}`,
      options?.placeLabel ? `Shelf: ${options.placeLabel}` : null,
    ]
      .filter(Boolean)
      .join(' · '),
    lines,
...
/** Merge several shelf documents into one FridgeReview payload. */
export function mergeShelfDocuments(
  parts: DocumentExtract[],
  title?: string,
): DocumentExtract | null {
  ...
  return {
    kind: 'fridge',
    title: title ?? 'Shelf walk companion',
    confidence: Math.min(0.9, avg),
    rawNotes: parts
      .map((p) => p.rawNotes)
      .filter(Boolean)
