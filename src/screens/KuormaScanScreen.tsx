...
import { analyzeKuormaImage } from '../lib/visionStub';
import { colors, radius, spacing } from '../theme/colors';

export function KuormaScanScreen() {
  ...
  async function analyze(demoVariant?: 'A' | 'B') {
    setBusy(true);
    try {
      const document = await analyzeKuormaImage(
        uri ?? 'demo',
        demoVariant ?? variant,
      );
  ...
  return (
    <View style={[styles.root, { paddingTop: spacing.md, paddingBottom: insets.bottom }]}>
      ...
      <Text style={styles.sub}>
        Photograph the delivery paper → extract lines → match catalog → confirm
        adds stock.
      </Text>
      ...
      <View style={styles.preview}>
        {uri ? (
          ...
          <Text style={styles.placeholder}>
            Photo of supplier delivery paper
          </Text>
        )}
      </View>
      ...
      <View style={styles.demoRow}>
        <Text style={styles.label}>Demo payload</Text>
        {(['A', 'B'] as const).map((v) => (
          <Pressable
            key={v}
      ...
      <Pressable
        ...
        disabled={busy}
        onPress={() => analyze(variant)}
      >
        {busy ? (
          <ActivityIndicator color="#fff" />
        ) : (
          <Text style={styles.analyzeText}>
            {uri ? 'Analyze delivery' : `Run delivery demo ${variant}`}
          </Text>
        )}
      </Pressable>
    </View>
...
const styles = StyleSheet.create({
  ...
  sub: { color: colors.inkMuted, marginTop: 6, marginBottom: spacing.md, lineHeight: 18 },
  preview: {
    height: 200,
  ...
  placeholder: { color: colors.inkMuted, textAlign: 'center', paddingHorizontal: 24 },
  ...
  label: { color: colors.inkMuted, fontWeight: '600', fontSize: 12, marginRight: 4 },
  chip: {
